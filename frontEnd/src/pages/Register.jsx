...
const Register = () => {
  ...
  const [login, setLogin] = useState({
   ...
   const [errors, setErrors] = useState({}); // State for error messages
    ...
    if (!login.full_name) {
      newErrors.full_name = 'Nama Lengkap is required';
    }
...
const handleInputChange = e => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(login)
        ...
        const response = await authController.register(login);
        ...
        if (response.error === 1) {
          ...
          setSnackbarMessage("Register Berhasil");
           setSnackbarOpen(true); // Open Snackbar on success
        ...
        <Box padding={'10px'}>
          <form onSubmit={handleSubmit} action=''>
            <FormControl>
              ...
              />
               {errors.full_name && <Typography fontSize={14} color='danger'>{errors.full_name}</Typography>}
            </FormControl>
            <FormControl>
